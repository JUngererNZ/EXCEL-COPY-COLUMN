...
- The worksheet name includes a trailing space.
- The script follows the same duplicate-right / hide-left workflow.
- The workbook is updated in place.

### `script-SURYA-MINES-TRACKING-2.py`

Configured for:

```python
input_file = Path(r"C:\Users\Jason\Projects\EXCEL-COPY-COLUMN\BARTRAC - SURYA MINES - TRACKING 21-04-2026.xlsx")
sheet_name = "CURRENT SHIPMENTS "
header_row = 1
```
...
- The worksheet name includes a trailing space.
- This script uses the same column copy and hide logic.
- The workbook is updated in place.

### `script-MUMI-TRACKING-3.py`

Configured for:

```python
input_file = Path(r"C:\Users\Jason\Projects\EXCEL-COPY-COLUMN\BARTRAC - MUMI TRACKING AS OF 31-12-2025.xlsx")
sheet_name = "ENROUTE SITE"
header_row = 7
```
...
- The header row is `7` for this workbook.
- The same regex matching and column duplication workflow applies.
- The workbook is updated in place.
...
## Key additions
The main README update is that it now documents the newer scripts as workbook-specific variants rather than a single generic tool. It also notes the different header rows and the trailing-space sheet names where applicable.
