...
SHEET_NOT_FOUND_EXIT_CODE = 3
HEADER_NOT_FOUND_EXIT_CODE = 4
...
def main():
    try:
        ...
        if not input_file.exists():
            print(f"Error: Workbook not found: {input_file}")
            return FILE_NOT_FOUND_EXIT_CODE
        ...
        if sheet_name not in wb.sheetnames:
            ...
            return SHEET_NOT_FOUND_EXIT_CODE

        ws = wb[sheet_name]

        # =========================
        # Find the LAST matching COMMENTS column
        # =========================
        target_col = None
        ...
        if target_col is None:
            print(f"Error: No header matching pattern 'COMMENTS DD-MM-YYYY' found in row {header_row}.")
            return HEADER_NOT_FOUND_EXIT_CODE
